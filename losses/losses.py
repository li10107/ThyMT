...
def IOULossWithPositionalWeight(input, target, positinal_weight):
    ...
    intersect = (input * target * (1 + positinal_weight)).sum()

    union = ((input + target - input * target) * (1 + positinal_weight)).sum()

    return 1. - torch.mean(intersect / union.clamp(min=epsilon))


def IOULossWithPositionalWeightMixUp(input, target, positinal_weight):
    ...
    intersect = (torch.sum(input * target, dim=1) * (1 + positinal_weight)).sum()

    union = (torch.sum((input + target - input * target), dim=1) * (1 + positinal_weight)).sum()
...
# 二分类 focal loss
def focal_loss_sigmoid(labels, y_pred, gamma=2, alpha=0.25):
    L = -labels * alpha * ((1 - y_pred) ** gamma) * torch.log(y_pred) - (1 - labels) * (y_pred ** gamma) * torch.log(1 - y_pred)
    return L
